  ...
  return (
    <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12">
      ...
      <div className="opacity-0 animate-scale-in">
        <div className="relative w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden ring-4 ring-blue-500/20 dark:ring-purple-500/20 shadow-xl">
          {data.avatarUrl ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-4xl md:text-5xl font-bold">
              {data.name.charAt(0)}
            </div>
          )}
        </div>
      ...
      <div className="flex flex-col gap-6 flex-1 text-center md:text-left">
        <h1 className="text-4xl md:text-6xl font-bold leading-tight gradient-text opacity-0 animate-fade-in-up">
          {data.heroName ?? data.name}
        </h1>
        <p className="text-lg md:text-xl text-zinc-600 dark:text-zinc-400 font-medium opacity-0 animate-fade-in-up delay-100">
          {data.role} • {data.location}
        </p>
        <p className="max-w-xl text-base md:text-lg text-zinc-700 dark:text-zinc-300 leading-relaxed opacity-0 animate-fade-in-up delay-200">
          {data.summary}
        </p>
        <div className="flex gap-3 mt-2 opacity-0 animate-fade-in-up delay-300 justify-center md:justify-start">
          <a
            href="#projects"
