...
export function ProjectDetailClient({ project }: ProjectDetailClientProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-white dark:from-zinc-950 dark:via-zinc-950 dark:to-zinc-950">
      {/* Header */}
      <header className="relative border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/80 backdrop-blur-sm sticky top-0 z-10 overflow-hidden">
        {/* Mesh gradient blobs for light mode */}
        <div className="absolute inset-0 overflow-hidden dark:hidden">
          <div className="absolute -top-10 -right-10 w-40 h-40 bg-gradient-to-br from-blue-400 to-cyan-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob"></div>
          <div className="absolute -bottom-10 -left-10 w-40 h-40 bg-gradient-to-br from-purple-400 to-pink-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-2000"></div>
        </div>
        <div className="relative max-w-4xl mx-auto px-4 py-4">
          ...
          >
            <span>←</span> {t("projects.backToPortfolio")}
          </Link>
        </div>
      </header>

      {/* Main Content */}
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-12 md:py-16 my-8 bg-white dark:bg-zinc-900/60 backdrop-blur-sm rounded-2xl border border-zinc-200 dark:border-zinc-800/60 shadow-lg dark:shadow-none">
        {/* Title & Meta */}
        ...
        <div className="mb-12 opacity-0 animate-fade-in-up delay-200">
          <h2 className="text-2xl font-bold mb-4 text-zinc-900 dark:text-zinc-100">🛠️ {t("projects.techStack")}</h2>
          <div className="flex flex-wrap gap-3">
            {project.techStack.map((tech) => (
              <span
                key={tech}
                className="px-4 py-2 bg-gradient-to-br from-zinc-100 to-zinc-200 dark:from-zinc-800 dark:to-zinc-900 border border-zinc-300 dark:border-zinc-700 rounded-lg font-medium text-sm text-zinc-800 dark:text-zinc-200 hover:scale-105 transition-transform"
              >
                {tech}
              </span>
            ))}
        ...
        {project.longDescription && (
          <div className="mb-12 opacity-0 animate-fade-in-up delay-300">
            <h2 className="text-2xl font-bold mb-4 text-zinc-900 dark:text-zinc-100">📖 {t("projects.overview")}</h2>
        ...
        {project.challenges && project.challenges.length > 0 && (
          <div className="mb-12 opacity-0 animate-fade-in-up delay-400">
            <h2 className="text-2xl font-bold mb-4 text-zinc-900 dark:text-zinc-100">🎯 {t("projects.challenges")}</h2>
        ...
        {project.solutions && project.solutions.length > 0 && (
          <div className="mb-12 opacity-0 animate-fade-in-up delay-500">
            <h2 className="text-2xl font-bold mb-4 text-zinc-900 dark:text-zinc-100">💡 {t("projects.solutions")}</h2>
            <div className="space-y-4">
        ...
        {project.architecture && (
          <div className="mb-12 opacity-0 animate-fade-in-up delay-600">
            <h2 className="text-2xl font-bold mb-4 text-zinc-900 dark:text-zinc-100">🏗️ {t("projects.architecture")}</h2>
